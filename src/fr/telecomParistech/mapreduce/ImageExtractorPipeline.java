...
public class ImageExtractorPipeline extends Job1<List<Entity>, String>{
	...
	public Value<List<Entity>> run(String mpdLocation) {
		...
		FutureValue<List<Entity>> entityList = 
				futureCall(new ParsingMpdJob(), mpd, immediate(mpdLocation));
		
		FutureValue<String> str = 
				futureCall(new Test(), entityList);
		
		return entityList;
	}


	private class Test extends Job1<String, List<Entity>> {

		@Override
		public Value<String> run(List<Entity> entityList) {
	...
	private class ParsingMpdJob extends Job2<List<Entity>, MPD, String> {
