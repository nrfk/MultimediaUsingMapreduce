...
public class BitmapHeader {
	...
	public void setAttribute(Attribute a, byte[] value) {
		...
		// Attribute of Dib Header, do an extra step
		if ( (offSet >= 0x0E) &&(offSet < 0x86)) {

			// Update DIB Header Size
			byte[] sizeInByte = attributes
					.get(Attribute
							.DIB_HEADER_SIZE
							.getOffset());

			// Update current size
			int size = ConvertUtility.byteArrayToInteger(	
					(byte[]) sizeInByte, 
					ByteOrder.LITTLE_ENDIAN);

			size += value.length;
			sizeInByte = ConvertUtility.integerToByteArray(
					size, 
					ByteOrder.LITTLE_ENDIAN);


			attributes.put(Attribute.DIB_HEADER_SIZE.getOffset(), sizeInByte);
		}

		// Update DIB Header Size
		byte[] sizeInByte = attributes.get(Attribute.FILE_SIZE.getOffset());

		// Update current size
		int size = ConvertUtility
				.byteArrayToInteger(	(byte[]) sizeInByte, 
						ByteOrder.LITTLE_ENDIAN);
		...
		attributes.put(Attribute.FILE_SIZE.getOffset(), sizeInByte);
	}
	...
	public byte[] dump() {
		...
		System.out.println("Length Att: " + lengAtt);
		System.out.println("Length : "  + headerLength);
		int offset = 0;
